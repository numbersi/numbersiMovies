...
module.exports = class  Crawler360kan{
  ...
  async searchApi(){
    ...
    const data =JSON.parse(/{.*}/i.exec(result)[0])
    if(data.errno){
      return "没找到"
    }
    return data
    // if( !data.data.detail.playlink.url.search('www.hanju.cc')){
    //   return "1没找到"

    // }
    const suglist=  data.data.suglist[0]
    const detail_key =suglist.type+'_'+suglist.info
    const playlink_url = data.data.detail[detail_key].playlink
