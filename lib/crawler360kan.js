...
module.exports = class Crawler360kan {
  ...
  async checkUpdate(href,md5_html){
    const url = this.baseUrl+href ;
    const html = await getHtml(url)
    const new_md5_html = md5(html)
    if(new_md5_html == md5_html){
      return null
    }
    return this.TvDetial(href)
  }
  ...
  async TvDetial(href) {
    ...
    const list_play = await switchsite({
      site,
      id,
      category
    })
...
function commonData($) {
  const img = $('#js-s-cover img').attr('src')
  const title = $('.title-left h1').text()
  ...
  let list_actor = []
  let list_cat = []
  $('.item-actor a.name').each(function () {
    let name = $(this).text()
